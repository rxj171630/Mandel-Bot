  ...
  async execute(client, interaction) {
    if (interaction.options.getSubcommand() === "random") {
      ...
      if (JSON.stringify(myJSON.strInstructions).length > 1900) {
        ...
        message = await interaction.channel.send(`\`\`\`${split2}\`\`\``);
      } else {
        message = await interaction.channel.send(`\`\`\`${instructions}\`\`\``);
      }

      setTimeout(
        () => message.react(`<:DrakeNo:1158286870661320725>`),
        ms("0.5 sec")
      );
      setTimeout(
        () => message.react(`<:DrakeYes:1158286868954222633>`),
        ms("0.5 sec")
      );
      ...
      // message.react(`<:drake_no:946723622632976404>`);
      // message.react(`<:drake_yes:946723680216555520>`);
    }
  },
};
