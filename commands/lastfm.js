...
async function fetchArt(id, myJson) {
  ...
  try {
    console.log(
      `song: ${myJson.similartracks.track[id].name} artist: ${myJson.similartracks.track[id].artist.name} ${myJsonArt.track.album.image[3]["#text"]}`
    );
    ...
    console.log(
      `song: ${myJson.similartracks.track[id].name} artist: ${myJson.similartracks.track[id].artist.name} ${art}`
    );
  }
  return art;
}
      ...
      for (i = 0; i < 5; i++) {
        await embeds.push(
          new EmbedBuilder()
            ...
            .setColor(`#ffffff`)
            .setDescription(
              `${myJson.similartracks.track[i].name} by ${myJson.similartracks.track[i].artist.name}`
            )
